...
(function () {
  ...
  // Epoch 4 wave 1 - designed, not built. Generated by tools/build-upcoming.mjs. See mergeUpcoming().
  var UPCOMING_URL = "data/upcoming.json";
  var WIKI_VERSION = "31";
  ...
  function itemList(d) {
    ...
    return d.items.slice().sort(function (a, b) {
      ...
      // Planned gear has NO authored stat, so itemPrimaryStat() reads 0 and it would sort to the
      // very front of its slot as if it were the weakest item in the game. Park it at the end.
  ...
  function loadData() {
    ...
    return fetch(DATA_URL, { cache: "no-cache" }).then(function (r) {
      ...
      // Planned content is merged AFTER localization on purpose: it has no localization keys yet,
      // so it stays in English rather than falling through the translator and coming out blank.
      // A missing or !ok upcoming.json is never fatal - the wiki just renders the live game.
      return fetch(UPCOMING_URL, { cache: "no-cache" })
        .then(function (r) { return r.ok ? r.json() : null; })
        .catch(function () { return null; })
        .then(function (u) {
          if (u) { try { mergeUpcoming(d, u); } catch (e) { d.upcoming = null; } }
  ...
  function live(arr) { return (arr || []).filter(function (x) { return !x.upcoming; }); }

  /* ---------- planned content (Epoch 4 / wave 1) ----------------------------------------------
   * Stone, Egypt and The Temple are DESIGNED but NOT BUILT: no Unity assets, no art, no authored
   * stats. data/upcoming.json is generated straight out of the game repo design doc by
   * tools/build-upcoming.mjs, so the wiki can never publish a name the setup tools will not use.
   *
   * NOTHING MERGED HERE MAY CARRY AN HP / ATK / DEF NUMBER. Levels ARE published - they are the
   * design doc projected band and the route is unreadable without them - but every one is labelled
   * "projected". A fabricated combat stat on a public wiki is worse than a blank, and it would be
   * read back later as if the balance pass had authored it.
   *
   * Everything merged carries upcoming:true; live() strips it wherever a count, or a question about
   * the live game, is what the page owes the reader.
   */
  function mergeUpcoming(d, u) {
    ...
    (u.worlds || []).forEach(function (w) {
      var world = w.name;
      var bandOf = {};
      (w.maps || []).forEach(function (m) { bandOf[m.id] = m.bandStart; });
      (w.maps || []).forEach(function (m) {
        ...
        d.areas.push({ code: m.id, mapId: m.id, name: m.name, world: world, upcoming: true,
          ...
          bossIds: isBossMap ? [w.boss.id] : [],
          dropItemIds: gear.map(function (g) { return g.id; })
            .concat(isBossMap && w.boss.relic ? [w.boss.relic.id] : []) });
      });
      (w.creatures || []).forEach(function (c) {
        var bands = c.areas.map(function (a) { return bandOf[a]; }).filter(function (v) { return v; });
        d.enemies.push({ id: c.id, name: c.name, image: "", isBoss: false, upcoming: true,
          minLevel: bands.length ? Math.min.apply(null, bands) : null,
          maxLevel: bands.length ? Math.max.apply(null, bands) : null,
          worlds: [world], areas: c.areas, zoneNames: [], drops: [], resists: null });
      });
      (w.gear || []).forEach(function (g) {
        d.items.push({ id: g.id, name: g.name, image: "", type: g.type, rarity: "", upcoming: true,
          description: "", setName: "", isUnique: false, isBossItem: false, isTrollItem: false,
          isHardModeItem: false, shopUnavailable: true, hiddenFromCollection: false,
          sourceRegion: g.tier, dropAreas: [g.tier], shopAreas: [], buyPrice: 0, effects: [] });
      });
      if (w.boss) {
        d.bosses.push({ id: w.boss.id, name: w.boss.name, image: "", mapId: w.boss.mapId,
          upcoming: true, level: w.boss.level, blurb: w.boss.blurb, relic: w.boss.relic,
          dropItemId: w.boss.relic ? w.boss.relic.id : "",
          dropItemName: w.boss.relic ? w.boss.relic.name : "" });
        if (w.boss.relic) {
          d.items.push({ id: w.boss.relic.id, name: w.boss.relic.name, image: "", type: "Accessory",
            rarity: "", upcoming: true, description: "", setName: "", isUnique: true,
            isBossItem: true, isTrollItem: false, isHardModeItem: false, shopUnavailable: true,
            hiddenFromCollection: false, sourceRegion: w.boss.mapId, dropAreas: [w.boss.mapId],
            shopAreas: [], buyPrice: 0, maxCopies: w.boss.relic.cap,
            effects: ["HP / ATK / DEF " + w.boss.relic.bonus + " (planned)"] });
        }
      }
    });
  }
  // Where a planned map's doors go. Straight out of Tools/epoch4_blueprint/maps.js, which is the
  // authored source for map geometry and is machine-verified against the engine constants — so
  // these are the real planned connections, not a guess drawn from the route order.
  var DOOR_KIND = {
    onward:    { icon: "&#8594;", label: "Onward" },
    back:      { icon: "&#8592;", label: "Back" },
    hub:       { icon: "&#8593;", label: "To the hub" },
    region:    { icon: "&#8594;", label: "Gateway" },
    ring:      { icon: "&#8596;", label: "Ring door" },
  ...
  function doorSection(d, m) {
    ...
    var rows = doors.map(function (dr) {
      ...
      return "<tr><td>" + k.icon + " " + esc(k.label) + "</td><td>" + cell +
        ' <small>' + esc(dr.to) + "</small></td><td>" + esc(place) + "</td></tr>";
    }).join("");
    return '<div class="section-title">Connections</div>' +
      '<table class="data"><tr><th>Door</th><th>Leads to</th><th>Placement</th></tr>' + rows + "</table>";
  }
  function upcomingBadge() { return '<span class="badge upcoming">Upcoming</span>'; }
  // The one line every planned card and detail shows in place of stats it does not have.
  var UPCOMING_NOTE = "Planned content \u2014 not yet in the game. No stats are authored.";
  ...
  PAGES.monsters = function (app, d) {
    ...
    listView(app, d, {
      items: list, page: "monsters.html", title: "Monsters",
      subtitle: live(list).length + " monsters (Normal + Hard)" +
        (list.length - live(list).length ? " + " + (list.length - live(list).length) + " upcoming" : ""),
      tabs: [
      ...
      card: function (e) {
        ...
        // Planned monsters have no authored HP/ATK. Say so - never print the 0 that fmt() returns.
        if (e.upcoming) return cardShell("monsters.html", e.id, e.image, e.name,
          upcomingBadge() +
          '<span class="badge">' + esc((e.worlds || [])[0] || "") + "</span>" +
          (where.length ? '<span class="badge">' + esc(where.join(", ")) + "</span>" : "") +
          '<span class="meta">' + (e.minLevel ? 'Projected Lv ' + rng(e.minLevel, e.maxLevel) + ' &#183; ' : '') + "stats not yet authored</span>");
  ...
  function monsterDetail(app, d, e, forcedMode) {
    ...
    if (e.upcoming) return upcomingDetail(app, d, "monsters.html", "Monsters", e, plannedRows([
      ["Appears on", areaNames(d, e.areas).join(", ")],
      ["Projected level", e.minLevel ? rng(e.minLevel, e.maxLevel) : ""]]));
  ...
  PAGES.items = function (app, d) {
    ...
    listView(app, d, {
      items: sorted, page: "items.html", title: "Items",
      subtitle: live(d.items).length + " items (Normal + Hard)" +
        (d.items.length - live(d.items).length ? " + " + (d.items.length - live(d.items).length) + " upcoming" : ""),
      tabs: [
      ...
      card: function (i) {
        ...
        if (i.upcoming) return '<a class="card rar item-card is-upcoming" href="items.html?id=' +
          ...
          '<div class="body"><h4>' + esc(i.name) + "</h4>" +
          '<div class="meta">' + upcomingBadge() + " " + esc(i.type) + "</div>" +
          '<div class="meta" style="margin-top:.25rem">Gear tier ' + esc(i.sourceRegion) +
          " &#183; stats not yet authored</div></div></a>";
  ...
  function itemDetail(app, d, i, forcedMode) {
    ...
    if (i.upcoming) return upcomingDetail(app, d, "items.html", "Items", i, plannedRows([
      ["Slot", i.type], ["Gear tier", i.sourceRegion],
      ["Drops from", areaNames(d, i.dropAreas).join(", ")],
      ["Planned bonus", (i.effects || [])[0] || ""]]));
  ...
  var WORLD_META = {
    ...
    // unwanted historical error"). Two shipped icons proved the point: Japan was U+26EA CHURCH,
    // and Korea was U+1F3EF JAPANESE CASTLE - a Japanese building representing Korea.
    // ⛔ These are CSS flags, not emoji. Windows ships no regional-indicator glyphs, so "\u{1F1EF}\u{1F1F5}"
    // renders as the bare letters "JP" there; .flag-us was already drawn in CSS for that reason.
    "Japan":      { icon: '<span class="flag-jp" role="img" aria-label="Japan flag"></span>', color: "#e0607a" },
    "Greek":      { icon: '<span class="flag-gr" role="img" aria-label="Greece flag"></span>', color: "#5bc8d8" },
    ...
    // Gate (the door that reads "Coming Soon" in game today). The Temple deliberately takes a
    // lantern rather than any religious icon: the region ships with no named figure and nothing
    // depicting a deity, and its art must not acquire one through the wiki either.
  ...
  function areaRows(d, w, mode) {
    ...
    return '<section class="area-group" style="--wc:' + meta.color + '">' +
      ...
      '<div class="area-rows">' + list.map(function (a) {
        ...
        if (a.upcoming) {
          var pf = (a.enemyIds || []).length
            ? [(a.enemyIds || []).length + " planned monsters",
               (a.dropItemIds || []).length + " planned drops"]
            : ["Hub town &#183; no encounters"];
          if ((a.bossIds || []).length) pf.push("1 boss");
          return '<a class="area-row is-upcoming" href="maps.html?id=' + encodeURIComponent(a.mapId) +
            '&mode=' + mode + '"><span class="area-code">' + esc(a.code) + "</span>" +
            '<span class="area-name">' + esc(a.name) + ' <span class="badge upcoming">Upcoming</span></span>' +
            '<span class="area-meta">' + (a.minLevel ? "Projected Lv " + fmt(a.minLevel) + " &#183; " : "") +
  ...
  PAGES.maps = function (app, d, forcedMode) {
    ...
    app.innerHTML =
      ...
      '<div class="world-view" data-panel="map"><div class="worldmap"><div class="wm-grid">' +
        ...
        '</div></div>' +
        '<div class="wm-conn h" style="grid-area:hwg"></div>' +
        '<div class="wm-conn v" style="grid-area:vwg"></div>' +
    ...
    (function (wm) {
      ...
      // Drag on BOTH axes. It used to move scrollLeft only, against an overflow-y:hidden canvas,
      // so anything below the fold could not be reached by dragging at all.
      var startX = 0, startY = 0, sl = 0, st = 0, panning = false, moved = false;
      wm.addEventListener("dragstart", function (e) { e.preventDefault(); });
      wm.addEventListener("pointerdown", function (e) {
        if (e.pointerType !== "mouse" || e.button !== 0) return;
        panning = true; moved = false;
        startX = e.clientX; startY = e.clientY; sl = wm.scrollLeft; st = wm.scrollTop;
        wm.classList.add("panning");
      });
      wm.addEventListener("pointermove", function (e) {
        if (!panning) return;
        var dx = e.clientX - startX, dy = e.clientY - startY;
        // 4px of slop so a click on a region node is never eaten as a drag.
  ...
  function worldView(app, d, w, forcedMode) {
    ...
    if (!route && s.maps.length && s.maps[0].upcoming) {
      ...
      var root = s.maps.filter(function (m) { return !seen[m.id]; })[0];
      if (root) route = { root: root.id, edges: edges };
    }
    ...
    function mnode(m) {
      ...
      // Planned maps carry no ZONES (nothing is baked), so mapEncounters() legitimately returns
      ...
      if (m.upcoming) {
        ...
        return '<a class="mnode is-upcoming" data-mid="' + esc(m.id) + '" href="maps.html?id=' +
          encodeURIComponent(m.id) + '&mode=' + mode + '" style="--wc:' + meta.color + '">' +
          thumb(m.image, m.name) + '<div class="mnode-body"><h4>' + esc(m.name) +
          ' <span class="badge upcoming">Upcoming</span></h4><div class="meta">' +
          pf.join(" &#183; ") + '</div></div></a>';
      }
  ...
  function mapDetail(app, d, m, forcedMode) {
    ...
    if (m.upcoming) {
      ...
      var drops = (ar.dropItemIds || []).length;
      return upcomingDetail(app, d, "maps.html", "Maps", m, plannedRows([
        ["Planned zones", m.plannedZones],
        ["Projected level", m.plannedLevel ? fmt(m.plannedLevel) : ""],
        ["Gear tier", drops ? drops + " items" : ""],
        ["Map size", m.plannedSize],
        ["Walked", m.plannedAxis]]) +
        doorSection(d, m) +
        ((m.plannedZoneNames || []).length
          ? '<div class="section-title">Zones, in travel order</div><div class="effect-list">' +
            m.plannedZoneNames.map(function (z, i) {
              return '<span class="fx">Z' + i + " " + esc(z) + "</span>";
            }).join("") + "</div>"
          : "") +
        (m.plannedNote ? '<div class="section-title">Design note</div><p>' + esc(m.plannedNote) + "</p>" : "") +
        (ar.enemyIds && ar.enemyIds.length ? '<div class="section-title">Planned monsters</div><p>' +
          linkList("monsters.html", ar.enemyIds, d._enemyById) + "</p>" : "") +
        (ar.bossIds && ar.bossIds.length ? '<div class="section-title">Planned boss</div><p>' +
          linkList("bosses.html", ar.bossIds, d._bossById) + "</p>" : "") +
        (ar.dropItemIds && ar.dropItemIds.length ? '<div class="section-title">Planned drops</div><p>' +
          linkList("items.html", ar.dropItemIds, d._itemById) + "</p>" : ""));
    }
    var mode = forcedMode || selectedGameMode("normal");
    rememberGameMode(mode, false);
  ...
  // One detail panel for every planned record. Deliberately a SEPARATE renderer rather than a set
  // of null-guards threaded through monsterDetail/bossDetail/itemDetail/mapDetail: those four read
  // dozens of stat fields, and any one missed guard would print a fabricated 0 on a public page.
  function upcomingDetail(app, d, page, label, x, rows) {
    var u = d.upcoming || {};
    var w = (u.worlds || []).filter(function (ww) {
      return ww.name === ((x.worlds && x.worlds[0]) || x.world) ||
             (x.id && String(x.id).indexOf(ww.code) === 0) ||
             (x.sourceRegion && String(x.sourceRegion).indexOf(ww.code) === 0) ||
             (x.mapId && String(x.mapId).indexOf(ww.code) === 0);
    })[0];
    app.innerHTML = backLink(page, label) +
      '<div class="detail">' + portrait(x.image, x.name) +
      "<div><h1>" + esc(x.name) + "</h1>" +
      '<div class="tags"><span class="pill upcoming">Upcoming</span>' +
        (w ? '<span class="pill">' + esc(w.name) + "</span>" : "") +
        (w && w.leg ? '<span class="pill">Route leg ' + w.leg + "</span>" : "") + "</div>" +
      '<div class="notice" role="status"><strong>' + esc(UPCOMING_NOTE) + '</strong><br>' +
        'Names and structure come from the design document; nothing is built yet, so there is no ' +
        'art and no combat data. Any level shown is a <em>projected</em> band, not an authored ' +
        'value, and it can still move before release.</div>' +
      (rows || "") +
      (w && w.pitch ? '<div class="section-title">The region</div><p>' + esc(w.pitch) + "</p>" : "") +
      "</div></div>";
  }
  function plannedRows(pairs) {
    var out = pairs.filter(function (p) { return p[1] != null && p[1] !== ""; })
      .map(function (p) { return sb(p[0], p[1]); }).join("");
    return out ? '<div class="statgrid">' + out + "</div>" : "";
  }
