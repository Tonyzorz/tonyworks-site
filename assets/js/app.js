  ...
  PAGES.maps = function (app, d, forcedMode) {
    ...
    app.innerHTML =
      ...
      '<div class="world-view" data-panel="map"><div class="worldmap"><div class="wm-grid">' +
        ...
        '<div class="wm-branch" style="grid-area:wg">' +
          // Tier 1 — the Maze, alone, directly beneath the hub, then its own three gate halls.
          '<div class="wm-branch-row">' +
            '<div class="wm-branch-item"><span class="wm-drop"></span>' + wnode(d, "Maze", { upcoming: mazeBatchUpcoming }) + "</div>" +
          ...
          '</div></div>' +
          // Tier 2 — the four themed regions. They are reached from the WORLD GATE, not through
          // the Maze above them, so they are captioned rather than wired into the Maze trunk.
          '<div class="wm-tier-gap"></div><span class="wm-tier-label">Also from the World Gate</span>' +
          '<span class="wm-tier-stem"></span><div class="wm-branch-bus"></div><div class="wm-branch-row">' +
          ...
          '</div></div>' +
      '</div></div><p class="route-note">Drag the map in any direction to look around. Grassland connects south to the World Gate. Directly below the hub is the Maze, which leads onward to Ice, America and Amazon; Japan, Greek, Military and Heaven sit on their own row below that, and each is reached from the World Gate itself rather than through the Maze. The door on the right side of the hub opens into the Graveyard. Korea, London and Monochrome hang from one rail beside the Graveyard: none is entered through another — each has its own door inside the Graveyard, and rising monster levels are what make Korea the first stop, London the second and Monochrome the finale. Void Hunt remains the secret arena reached from Volcanic. The dashed rail on the left is <strong>not in the game yet</strong>: the hub&#8217;s west door opens onto the planned Cloud Plaza, a second hub town, with Stone, Egypt and The Temple beyond it. Those three also connect to one another, so the circle can be walked in either direction once it exists &#8212; but each is entered from the plaza first. Nothing there is built, and no combat data for it exists.</p></div>' +
